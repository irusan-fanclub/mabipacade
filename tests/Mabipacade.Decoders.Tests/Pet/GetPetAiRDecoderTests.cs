...
public class GetPetAiRDecoderTests
{
    ...
    {
        Assert.Equal((uint)0xA8A3, new GetPetAiRDecoder().Op);
    }
    ...
    public void Decodes_Sample()
    {
        ...
        var input = new DecoderInput(DateTime.UtcNow, Direction.Inbound, 0xA8A3, 4504699139850743UL, elems);
    ...
    public void Decodes_NegativeAnswerOmitsString()
    {
        ...
        var input = new DecoderInput(DateTime.UtcNow, Direction.Inbound, 0xA8A3, 4504699144746549UL, elems);
        var result = Assert.IsType<GetPetAiR>(new GetPetAiRDecoder().Decode(input));
