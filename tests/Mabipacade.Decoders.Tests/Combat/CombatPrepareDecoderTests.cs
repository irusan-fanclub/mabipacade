...
{
    [Fact]
    public void Op_Matches() => Assert.Equal((uint)0x00007919, new CombatPrepareDecoder().Op);

    [Fact]
    public void Decodes_Sample()
    {
        var elems = new List<MessageElem> { MessageElem.Byte(1), MessageElem.Long(4767482419982230) };
        var input = new DecoderInput(DateTime.UtcNow, Direction.Inbound, 0x00007919, 0UL, elems);
