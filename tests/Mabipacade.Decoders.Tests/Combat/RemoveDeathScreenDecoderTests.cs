...
{
    [Fact]
    public void Op_Matches() => Assert.Equal((uint)0x53FD, new RemoveDeathScreenDecoder().Op);

    [Fact]
    public void Decodes_Empty()
    {
        var input = new DecoderInput(DateTime.UtcNow, Direction.Inbound, 0x53FD, 0UL, new List<MessageElem>());
