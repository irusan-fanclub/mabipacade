...
{
    [Fact]
    public void Op_Matches() => Assert.Equal((uint)0x7922, new SetFinisher2Decoder().Op);

    [Fact]
    public void Decodes_Empty()
    {
        var input = new DecoderInput(DateTime.UtcNow, Direction.Inbound, 0x7922, 0UL, new List<MessageElem>());
