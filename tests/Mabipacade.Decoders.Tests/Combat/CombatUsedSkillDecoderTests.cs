...
{
    [Fact]
    public void Op_Matches() => Assert.Equal((uint)0x00007927, new CombatUsedSkillDecoder().Op);

    [Fact]
    public void Decodes_Sample()
    {
        var elems = new List<MessageElem> { MessageElem.Short(24101) };
        var input = new DecoderInput(DateTime.UtcNow, Direction.Inbound, 0x00007927, 0UL, elems);
