...
{
    [Fact]
    public void Op_Matches() => Assert.Equal((uint)0x7D01, new CombatAttackRDecoder().Op);

    [Fact]
    public void Decodes_Sample()
    {
        var elems = new List<MessageElem> { MessageElem.Byte(1) };
        var input = new DecoderInput(DateTime.UtcNow, Direction.Inbound, 0x7D01, 0UL, elems);
