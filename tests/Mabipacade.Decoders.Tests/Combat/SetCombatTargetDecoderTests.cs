...
{
    [Fact]
    public void Op_Matches() => Assert.Equal((uint)0x00007920, new SetCombatTargetDecoder().Op);
    ...
    public void Decodes_Sample()
    {
        ...
        var input = new DecoderInput(DateTime.UtcNow, Direction.Inbound, 0x00007920, 0UL, elems);
    ...
    public void Handles_Empty() =>
        Assert.Equal(0UL, ((SetCombatTarget)new SetCombatTargetDecoder().Decode(
            new DecoderInput(DateTime.UtcNow, Direction.Inbound, 0x00007920, 0UL, new List<MessageElem>()))).TargetId);
}
